...
exports.MD2_HMAC = exports.MD2 = void 0;
var S = [
...
function md2(message) {
    ...
    var code, i, j, k, t, L = 0, loop = 1, B, index = 0, start = 0, bytes = 0, length = message.length;
    for (i = 0; i < 16; ++i) {
        X[i] = C[i] = 0;
    }
    M[16] = M[17] = M[18] = 0;
 ...
 */
function MD2(message, format = arrayType()) {
 ...
 * @returns `string|Uint8Array|Buffer`
 */
function MD2_HMAC(message, key, format = arrayType()) {
    const key_length = 16;
    const hash_len = 16;
    ...
    result.set(MD2(msg, "array"), key_length);
    var digestbytes = MD2(result);
    if (format == "hex") {
        return bytesToHex(digestbytes);
    }
    else if (format == "buffer") {
