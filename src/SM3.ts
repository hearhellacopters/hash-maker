 ...
 */
export function _SM3(message: InputData, format: OutputFormat = arrayType(), rounds: number = 64): string | Uint8Array | Buffer {
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function SM3_HMAC(message: InputData, key: InputData, format: OutputFormat = arrayType(), rounds: number = 64): string | Uint8Array | Buffer {
    const key_length = 64;
    const hash_len = 64;
    key = Hasher.convertToUint8Array(key);
    message = Hasher.convertToUint8Array(message);
    if (key.length > key_length) {
        key = _SM3(key, "array", rounds) as Uint8Array;
    }
    ...
    result.set(_SM3(msg, "array", rounds) as Uint8Array, key_length);

    var digestbytes = _SM3(result, "array", rounds) as Uint8Array;
    if (format == "hex") {
        return bytesToHex(digestbytes);
    } else if (format == "buffer") {
        return Buffer.from(digestbytes);
