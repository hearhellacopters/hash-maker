 ...
 */
export class HighwayHash {
  private v0 = new BigInt64Array(4);
  private v1 = new BigInt64Array(4);
  private mul0 = new BigInt64Array(4);
  private mul1 = new BigInt64Array(4);
   ...
   */
  public updatePacket(packet: Uint8Array, pos: number) {
    if (pos < 0) {
   ...
   */
  public update(a0: bigint, a1: bigint, a2: bigint, a3: bigint) {
    if (this.done) {
   ...
   */
  public updateRemainder(bytes: Uint8Array, pos: number, size_mod32: number) {
    if (pos < 0) {
      throw new Error(`Pos (${pos}) must be positive`);
    }
  ...
  public finalize256(): Uint8Array {
    ...
    this.modularReduction(
      this.v1[1] + this.mul1[1], this.v1[0] + this.mul1[0],
      this.v0[1] + this.mul0[1], this.v0[0] + this.mul0[0],
      hash, 0);
    this.modularReduction(
      this.v1[3] + this.mul1[3], this.v1[2] + this.mul1[2],
      this.v0[3] + this.mul0[3], this.v0[2] + this.mul0[2],
      hash, 2);
  ...
  private reset(key0: bigint, key1: bigint, key2: bigint, key3: bigint) {
  ...
  private zipperMerge0(v1: bigint, v0: bigint) {
  ...
  private zipperMerge1(v1: bigint, v0: bigint): bigint {
  ...
  private read64(buf: Uint8Array, off: number) {
    let value = BigInt(0);
    let endian = "little";
  ...
  private write64(val: bigint, buf: Uint8Array, off: number) {
    let endian = "little";
    let unsigned: boolean = false;
  ...
  private rotate32By(count: number, lanes: BigInt64Array) {
    for (let i = 0; i < 4; ++i) {
  ...
  private permuteAndUpdate() {
    this.update(lshr(this.v0[2], 32) | (this.v0[2] << BigInt(32)),
      lshr(this.v0[3], 32) | (this.v0[3] << BigInt(32)),
      lshr(this.v0[0], 32) | (this.v0[0] << BigInt(32)),
      lshr(this.v0[1], 32) | (this.v0[1] << BigInt(32)));
  }

  private modularReduction(a3_unmasked: bigint, a2: bigint, a1: bigint,
    a0: bigint, hash: BigInt64Array, pos: number) {
   ...
   */
  public static hash64(data: Uint8Array, offset: number = 0, length: number = data.length, key: Uint8Array) {
   ...
   */
  public static hash128(data: Uint8Array, offset: number = 0, length: number = data.length, key: Uint8Array) {
    const h = new HighwayHash(key);
    h.processAll(data, offset, length);
   ...
   */
  public static hash256(data: Uint8Array, offset: number = 0, length: number = data.length, key: Uint8Array) {
    const h = new HighwayHash(key);
    h.processAll(data, offset, length);
    return h.finalize256();
  }

  public processAll(data: Uint8Array, offset: number = 0, length: number = data.length) {
    var i;
 ...
 */
export function _HIGHWAY(message: InputData, key?: InputData, bitLen: 64 | 128 | 256 = 128, format: OutputFormat = arrayType()) {
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function HIGHWAY64(message: InputData, key?: InputData, format: OutputFormat = arrayType()) {
  var hash = new HighwayHash(formatMessage(key));
  hash.processAll(formatMessage(message));
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function HIGHWAY128(message: InputData, key?: InputData, format: OutputFormat = arrayType()) {
  var hash = new HighwayHash(formatMessage(key));
  hash.processAll(formatMessage(message));
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function HIGHWAY256(message: InputData, key?: InputData, format: OutputFormat = arrayType()) {
  var hash = new HighwayHash(formatMessage(key));
  hash.processAll(formatMessage(message));
...
export class HIGHWAY {
  static Highway = HighwayHash;
  static HIGHWAY = _HIGHWAY;
  static HIGHWAY64 = HIGHWAY64;
  static HIGHWAY128 = HIGHWAY128;
  static HIGHWAY256 = HIGHWAY256;
  /**
