...
function strToBuf(message?: InputData): Uint8Array {
    if (message === undefined) {
        return new Uint8Array(0);
    }
...
function bufToHex(bytes: Uint8Array = new Uint8Array(0)): string {
...
function x64hash128(
  buf: InputData = new Uint8Array(0),
  state: u32 | x64hash128State = 0x0,
  finalize: boolean = true,
): Uint8Array | x64hash128State {
  let str;
  if (typeof buf === 'string') {
    buf = strToBuf(buf);
    str = true;
 ...
 */
export function MURMUR3_X64_128(message: InputData, seed: number = 0, format: OutputFormat = arrayType()){
  const digestbytes = x64hash128(formatMessage(message), seed, true) as Uint8Array;
  if (format == "buffer") {
 ...
 * @returns `number`
 */
export function MURMUR3_X86_32(message: InputData, seed: number = 0){
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function MURMUR3_X86_128(message: InputData, seed: number = 0, format: OutputFormat = arrayType()){
  const digestbytes = x86hash128(formatMessage(message), seed, true) as Uint8Array;
  if (format == "buffer") {
		return Buffer.from(digestbytes);
	} else if (format == "hex") {
		return bufToHex(digestbytes);
