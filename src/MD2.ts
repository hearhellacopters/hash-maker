...
var M: number[] = [], X: number[] = [], C: number[] = [];

function md2(message: string | Uint8Array | Buffer) {
    ...
    return new Uint8Array(X.slice(0, 16));
};
 ...
 */
export function MD2(message: InputData, format: OutputFormat = arrayType()): Uint8Array | Buffer | string {
    const digestbytes = md2(message);
    if (format == "hex") {
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function MD2_HMAC(message: InputData, key: InputData, format: OutputFormat = arrayType()): Uint8Array | Buffer | string {
    const key_length = 16;
    const hash_len = 16;
    key = formatMessage(key);
    message = formatMessage(message);
    if (key.length > key_length) {
        key = MD2(key, "array") as Uint8Array;
    }
    ...
    result.set(MD2(msg, "array") as Uint8Array, key_length);

    var digestbytes = MD2(result) as Uint8Array;
    if (format == "hex") {
        return bytesToHex(digestbytes);
    } else if (format == "buffer") {
        return Buffer.from(digestbytes)
