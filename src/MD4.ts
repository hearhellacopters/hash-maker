...
// Interface for MD4 context
interface SphMD4Context {
    val: Uint32Array; // State (4 x 32-bit words)
    count: bigint; // Message length in bits
    buf: Uint8Array; // 64-byte buffer for input data
}

// MD4 round function
function md4Round(data: Uint8Array, r: Uint32Array): void {
...
function sphMD4Init(): SphMD4Context {
    return {
...
function sphMD4Update(sc: SphMD4Context, data: string | Uint8Array | Buffer): void {
...
function sphMD4Close(sc: SphMD4Context): Uint8Array {
 ...
 */
export function MD4(message: InputData, format: OutputFormat = arrayType()): Uint8Array | Buffer | string {
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function MD4_HMAC(message: InputData, key: InputData, format: OutputFormat = arrayType()): Uint8Array | Buffer | string {
    const key_length = 64;
    const hash_len = 16;
    key = formatMessage(key);
    message = formatMessage(message);
    if (key.length > key_length) {
        key = MD4(key, "array") as Uint8Array;
    }
    ...
    result.set(MD4(msg, "array") as Uint8Array, key_length);

    var digestbytes = MD4(result) as Uint8Array;
    if(format == "hex"){
        return bytesToHex(digestbytes);
    } else if(format == "buffer"){
        return Buffer.from(digestbytes);
