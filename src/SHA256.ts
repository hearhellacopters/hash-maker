 ...
 */
export function SHA256(message: InputData, format: OutputFormat = arrayType()) {
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function SHA256_HMAC(message: InputData, key: InputData, format: OutputFormat = arrayType()) {
    const key_length = 64;
    const hash_len = 32;
    key = formatMessage(key);
    message = formatMessage(message);
    if (key.length > key_length) {
        key = SHA256(key, "array") as Uint8Array;
    }
    ...
    result.set(SHA256(msg, "array") as Uint8Array, key_length);

    var digestbytes = SHA256(result, "array") as Uint8Array;
    if (format == "hex") {
        return bytesToHex(digestbytes);
    } else if (format == "buffer") {
        return Buffer.from(digestbytes);
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function SHA256_DOUBLE(message: InputData, format: OutputFormat = arrayType()) {
    var message2 = formatMessage(message);
    ...
    var message3 = wordsToBytes(H);

    H = [0x6A09E667, 0xBB67AE85, 0x3C6EF372, 0xA54FF53A,
        0x510E527F, 0x9B05688C, 0x1F83D9AB, 0x5BE0CD19];

    m = bytesToWords(new Uint8Array(message3));
    l = message3.length * 8;
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function SHA224(message: InputData, format: OutputFormat = arrayType()) {
    var message2 = formatMessage(message);
    ...
    var m = bytesToWords(new Uint8Array(message2));
    var l = message2.length * 8;
    m[l >> 5] |= 0x80 << (24 - l % 32);
    m[((l + 64 >> 9) << 4) + 15] = l;
 ...
 * @returns `string|Uint8Array|Buffer`
 */
export function SHA224_HMAC(message: InputData, key: InputData, format: OutputFormat = arrayType()) {
    const key_length = 64;
    const hash_length = 28;
    key = formatMessage(key);
    message = formatMessage(message);
    if (key.length > key_length) {
        key = SHA224(key) as Uint8Array;
    }
    ...
    result.set(outerKey, 0);
    result.set(SHA224(msg) as Uint8Array, key_length);

    var digestbytes = SHA224(result) as Uint8Array;
    if (format == "hex") {
        return bytesToHex(digestbytes);
    } else if (format == "buffer") {
        return Buffer.from(digestbytes);
